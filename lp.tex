...
Para começar a análise desse problema, precisamos identificar quais proposições serão representadas pelas letras. Como nossa principal meta é descobrir quem veste qual cor, usaremos : AA (Ana veste azul),AB (Ana veste branco), AP (Ana veste preto), MA (Maria veste azul), MB (Maria veste branco), MP (Maria veste preto), CA(Cláudia veste azul),CB (Cláudia veste branco),CP (Cláudia veste preto).
...
\end{itemize}
No começo do problema, nos é dito que as três irmãs foram para festa com cores diferentes de vestido, logo, sabemos que, se sabemos de uma irmã foi de determinada cor, as outras duas certamente não vestiram aquela cor. Assim:
\begin{itemize}
    \item (AA → (¬ MA ∧ ¬ CA)) ∧ (AB → (¬ MB ∧ ¬ CB)) ∧ (AP → (¬ MP ∧ ¬ CP))
    \item (MA → (¬ AA ∧ ¬ CA)) ∧ ((MB → (¬ AB ∧ ¬ CB)) ∧ (MP → (¬ AP ∧ ¬ CP)))
    \item (CA → (¬ AA ∧ ¬ MA)) ∧ (CB → (¬ AB ∧ ¬ MB)) ∧ (CP → (¬ AP ∧ ¬ MP))
\end{itemize}
Vamos agora analisar as sentenças ditas pelas irmãs, sabendo suas características individuais:
\bigbreak
Se Ana está de azul, sabemos sua sentença é verdadeira e, portanto, Ana está de branco. Escrevemos então:
\begin{itemize}
	\item AA → AB
\end{itemize}
Se Ana está de branco, sabemos sua sentença é verdadeira e, portanto, Maria está de branco. Escrevemos então:
\begin{itemize}
	\item AB → MB
\end{itemize}
Se Ana está de preto, sabemos sua sentença é verdadeira e, portanto, Cláudia é quem está de branco. Escrevemos então:
\begin{itemize}
	\item AP → CB
\end{itemize}
Se Claudia está de azul, sabemos sua sentença é falsa e, portanto, Ana não está de branco. Escrevemos então:
\begin{itemize}
	\item CA → ¬ AB
\end{itemize}
Se Claudia está de branco, sabemos sua sentença é falsa e, portanto, Maria não está de branco. Escrevemos então:
\begin{itemize}
	\item CB → ¬ MB
\end{itemize}
Se Claudia está de preto, sabemos sua sentença é falsa e, portanto, Cláudia não está de branco. Escrevemos então:
\begin{itemize}
	\item CP → ¬ CB
\end{itemize}
...
\end{itemize}
\bigbreak
Por meio de manipulações das fórmulas descritas acima, convidamos  o leitor nos exercícios a chegar na resposta encontrada no início deste capítulo.
