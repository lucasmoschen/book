...
Como já visto em capítulos anteriores, existe uma forma de fazer cálculos matemáticos em lean: utilizando o identificador {\fontencoding{U}\fontfamily{cmtt}\selectfont calc}. Um exemplo super simples do uso desse método em conjuntos é o seguinte:
...
\item $\overline {\mathcal U} = \emptyset$ : {\fontencoding{U}\fontfamily{cmtt}\selectfont compl\_univ}
\item $\overline {\emptyset} = \mathcal U$ : {\fontencoding{U}\fontfamily{cmtt}\selectfont compl\_empty}
\item $A \cap \overline A = \emptyset$ : {\fontencoding{U}\fontfamily{cmtt}\selectfont inter\_compl\_self} ({\fontencoding{U}\fontfamily{cmtt}\selectfont compl\_inter\_self})
\item $A \cup \overline A = \mathcal U$ : {\fontencoding{U}\fontfamily{cmtt}\selectfont union\_compl\_self} ({\fontencoding{U}\fontfamily{cmtt}\selectfont compl\_union\_self})
\item $\overline{\overline A} = A$ : {\fontencoding{U}\fontfamily{cmtt}\selectfont compl\_compl}
...
\item $\overline{(A \cap B)} = \overline A \cup \overline B$ : {\fontencoding{U}\fontfamily{cmtt}\selectfont compl\_inter}
\item $\overline{(A \cup B)} = \overline A \cap \overline B$ : {\fontencoding{U}\fontfamily{cmtt}\selectfont compl\_union}
\item $A \setminus B = A \cap \overline B$ : {\fontencoding{U}\fontfamily{cmtt}\selectfont diff\_eq}
\end{itemize}
...
\subsection{Em Lean}
Sendo {\fontencoding{U}\fontfamily{cmtt}\selectfont I U : Type}, nós podemos especificar que {\fontencoding{U}\fontfamily{cmtt}\selectfont A} é uma família de conjuntos indexados escrevendo {\fontencoding{U}\fontfamily{cmtt}\selectfont A : I → set U}. Ou seja, {\fontencoding{U}\fontfamily{cmtt}\selectfont A} é uma função tal que para cada elemento {\fontencoding{U}\fontfamily{cmtt}\selectfont i} do tipo {\fontencoding{U}\fontfamily{cmtt}\selectfont I}, ela retorna um conjunto {\fontencoding{U}\fontfamily{cmtt}\selectfont A i}. Acerca da notação em lean, ainda temos:

\begin{itemize}
  \item $x \in \bigcap_{i \in I} A_i \rightarrow$ {\fontencoding{U}\fontfamily{cmtt}\selectfont x ∈ Union A} ou {\fontencoding{U}\fontfamily{cmtt}\selectfont x ∈ ⋃ i, A i}
  
  Ambas as formas possuem uma interpretação de {\fontencoding{U}\fontfamily{cmtt}\selectfont ∀ i, x ∈ A i}

  \item $\bigcap \rightarrow$ {\fontencoding{U}\fontfamily{cmtt}\selectfont $\backslash$I} ou {\fontencoding{U}\fontfamily{cmtt}\selectfont $\backslash$bigcap}
  \item $x \in \bigcup_{i \in I} A_i \rightarrow$ {\fontencoding{U}\fontfamily{cmtt}\selectfont x ∈ Inter A} ou {\fontencoding{U}\fontfamily{cmtt}\selectfont x ∈ ⋂ i, A i}
  
  Ambas as formas possuem uma interpretação de {\fontencoding{U}\fontfamily{cmtt}\selectfont ∃ i, x ∈ A i}

  \item $\bigcup \rightarrow$ {\fontencoding{U}\fontfamily{cmtt}\selectfont $\backslash$Un} ou {\fontencoding{U}\fontfamily{cmtt}\selectfont $\backslash$bigcup}
\end{itemize}
...
É válido lembrar que às vezes queremos trabalhar com famílias $(A_{i,j})_{i \in I, j \in J}$ indexadas por duas variáveis. Isso continua fácil de se trabalhar no lean, só precisamos definir {\fontencoding{U}\fontfamily{cmtt}\selectfont A : I → J → set U}, significando que dado {\fontencoding{U}\fontfamily{cmtt}\selectfont i : I} e {\fontencoding{U}\fontfamily{cmtt}\selectfont j : J}, obtemos um conjunto {\fontencoding{U}\fontfamily{cmtt}\selectfont A i j : set U}. Aqui está um exemplo de uma prova envolvendo família indexada por duas variáveis:
...
\textbf{OBS:} Você pode encontrar um problema quando reproduzir esses códigos no lean, e para evitar isso, lembre-se que todos os arquivos precisam ter as duas primeiras linhas (para importar e abrir as bibliotecas). Mas além disso, outro conflito possível é sobre as definições de variáveis, pois na parte de teoremas definimos a variável {\fontencoding{U}\fontfamily{cmtt}\selectfont A}, e depois nos exemplos definimos ela novamente, o que causa um conflito. Para evitar isso, você pode simplesmente não utilizar o mesmo nome de variável duas vezes, ou então definir um escopo local para elas, utilizando {\fontencoding{U}\fontfamily{cmtt}\selectfont section} e {\fontencoding{U}\fontfamily{cmtt}\selectfont end}, respectivamente antes e depois dos seus exemplos/teoremas.

{\fontencoding{U}\fontfamily{cmtt}\selectfont }
...
show false, from h1 h3\end{lstlisting}
...
(assume x,
  ...
  show x ∈ (A ∩ B) ∩ C, from and.intro h₂ h₁.right)\end{lstlisting}
...
show A ⊆ B, from h3 (subset.refl A)\end{lstlisting}
