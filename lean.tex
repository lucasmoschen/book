...
Abrimos uma discussão sobre algumas noções úteis no processo de prova utilizando Lean; não esperamos que o leitor a essa altura entenda amplamente as provas dadas, e sim, que tenha uma noção de como funciona uma prova.

Quando desejamos iniciar a prova explicita para um assertiva, podemos abrir contextos de prova usando as palavras-reservadas \textit{example}, \textit{lemma} ou \textit{theorem}. Cada uma dessas abre um contexto de prova para uma assertiva desejada:
...
\noindent Para cada contexto, foram dados os argumentos a serem considerados no processo de prova. Na linha 2, por exemplo, exlicitamos que $A$ e $B$ são as proposições, $h1$ e $h2$ são as provas que deveriamos considerar no contexto.

Note que a prova é identica; a diferença é o produto após a prova feita. O \textit{theorem} assim como \textit{lemma} permitem fazer a prova de uma assertiva, e esse resultado poderá ser reutilizado, eventualmente; isso exige um titulo para o teorema ou lema provado.
Já \textit{example} funciona como um contexto fechado, ou rascunho: podemos realizar a prova, mas não nos referir a ela posteriromente.
\vspace{5mm}
\begin{lstlisting}
variables A B : Prop    -- A e B sao proposicoes
variable h1 : A         -- h1 uma prova para A
...
Ao fazermos uso do teorema, linha 8, tivemos de passar explicitamente (aplicar) as proposições, e as provas que temos que o teorema identificou como hipóteses para a prova.

Lean é inteligente o bastante para livrar o usuario desse trabalho. Se declaramos as proposições como argumentos implicitos, o programa será capaz de identificar a partir das hipoteses, quais as proposições:
...
\noindent Esse tipo de método tambem permite a generalização do resultado obtido. De fato, o teorema vale para quaisquer proposições $U$ e $V$ que se deseje provar. Quando pusemos chaves, $\{U V : Prop\}$ na declaração do teorema na linha 2, o próprio programa considerou como argumentos que poderiam ser compreendidos no contexto em que fosse utilizado.
...
\section{Aspectos Práticos}
Faz sentido falarmos em aspectos práticos sobre o Lean; onde ele está disponivel online, ou como conhecer os teoremas básicos disponíveis, sugestões para instalação, e uso.
